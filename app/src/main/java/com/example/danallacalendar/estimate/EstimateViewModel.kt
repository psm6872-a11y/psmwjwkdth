...
class EstimateViewModel @Inject constructor(
    ...
    private fun loadAndCopyEstimateFromJson(estimateJson: String) {
        try {
            android.util.Log.d("EstimateViewModel", "[COPY] Decoding and parsing estimateJson directly (Synchronously)...")
            val decodedJson = try {
            ...
            // 모든 텍스트/입력 필드를 우선 빈 값으로 안전하게 초기화 (기존 값 누적 방지)
            customerName.value = ""
            phoneNumber.value = ""
            departure.value = ""
            destination.value = ""
            ...
            workersM.value = "4"
            workersF.value = "1"
            laddersStartFloor.value = ""
            laddersStartCost.value = ""
            laddersEndFloor.value = ""
            laddersEndCost.value = ""
            extraTruck.value = ""
            moveCost.value = ""
            totalCost.value = ""
            deposit.value = "10"
